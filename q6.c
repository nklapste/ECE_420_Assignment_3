...
/*****************************************************************/
void Make_numbers(long int big_array[]  /* out */, int n /* in  */, int n_bar /* in  */, int p /* in  */) {
    ...
    for (q = 0; q < p; q++) {
        ...
        for (i = 0; i < n_bar; i++) {
            ...
            /* Assuming the range of elements in big_array is [0, 2n]*/
...
/*****************************************************************/
void Sequential_sort(long int array[] /* in/out */, int size /* in  */) {
...
/*****************************************************************/
void Put_numbers_in_bucket(long int big_array[] /* in */, long int local_array[] /* out */, int n /* in  */,
                           int n_bar /* in  */, int p /* in  */, int my_rank /* in  */) {
    /* Assume that numbers in big_array are evenly distributed at root,
       but are unsorted.  Send numbers to the process (bucket that should have them. This version uses unsafe messaging and may fail in some cases!!    */
    int i, q, bucket;
    MPI_Status status;
    if (my_rank == 0) {
        for (i = 0; i < n; i++) {
            bucket = big_array[i] / (2 * n_bar);  /* Assuming the range of the elements in big_array is [0, 2n] */
